...
function pd_meta_box_cb($post) {
	...
	if (isset($custom['buylink'][0])) { $buylink = $custom['buylink'][0]; }
	...
	<p>
		<label for="instock_text">buylink</label> 
		<input type="text" name="buylink" id="buylink" value="<?php if(!empty($buylink)) echo $buylink; ?>" />
	</p>
...
function pd_meta_box_save( $post_id ) {
	...
	if (isset($_POST['buylink']))
		update_post_meta( $post_id, 'buylink', esc_attr( $_POST['buylink'] ) );
